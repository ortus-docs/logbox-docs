...
| `void configure( any config )`          | Dynamically re-configure the LogBox library. Accepts a `LogBoxConfig` object, a plain class path (string) to a config class, or a struct literal (same as the `config` argument to the LogBox constructor). |
